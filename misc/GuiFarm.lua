--[[
	Elerium UI Library (imgui)
	by Singularity (V3rm @ King Singularity) (Discord @ Singularity#5490)
	reworked by Blitz (Discord @ blitzisking#0)

	stuff reworked / fixed:

	>> Sliders are set to values instead of percentages
	>> Draggability is fixed (for mobile and PC)
	>> More comming soon
--]]
				...
				do -- Tab Elements
					...
					function tab_data:AddSlider(slider_text, callback, slider_options)
						...
						do -- Slider Math
									...
									spawn(function() -- Loop check
										if Entered and not slider_options.readonly then
											while Held and (not dropdown_open) do
												...
												-- Get the mouse location relative to the slider
												local mouse_location = gMouse()
												local x = (mouse_location.X - slider.AbsolutePosition.X) / slider.AbsoluteSize.X -- Normalized position between 0 and 1

												-- Clamp x to ensure it stays between 0 and 1
												x = math.max(0, math.min(x, 1))

												-- Calculate the actual value in the range
												local minv = slider_options.min
												local maxv = slider_options.max
												local value_in_range = math.floor(x * (maxv - minv) + minv) -- Calculate the actual value between min and max

												-- Update the slider's indicator size
												Resize(indicator, {Size = UDim2.new(x, 0, 0, 20)}, options.tween_time)

												-- Update the displayed value
												value.Text = tostring(value_in_range)

												-- Call the callback function with the new value
												pcall(callback, value_in_range)
							...
							-- Set function to update slider programmatically
							function slider_data:Set(new_value)
								-- Ensure the value is numeric and within the range
								new_value = tonumber(new_value) or slider_options.min
								new_value = math.max(slider_options.min, math.min(slider_options.max, new_value)) -- Clamp between min and max

								-- Normalize the value to a 0-1 range
								local minv = slider_options.min
								local maxv = slider_options.max
								local normalizedValue = (new_value - minv) / (maxv - minv)
								normalizedValue = math.max(0, math.min(normalizedValue, 1)) -- Clamp between 0 and 1

								-- Update the slider's indicator size based on the normalized value
								Resize(indicator, {Size = UDim2.new(normalizedValue, 0, 0, 20)}, options.tween_time)

								-- Calculate and set the display value
								local displayValue = math.floor(normalizedValue * (maxv - minv) + minv)
								value.Text = tostring(displayValue)
